# Acknowledgement: Modified from OpenEvolve (https://github.com/codelion/openevolve/blob/main/openevolve/evaluator.py) under Apache-2.0 license
...
from ..optimizers.alphaevolve.ensemble import LLMEnsemble
from ..prompts.optimizers.alphaevolve_optimizer import (
...
class AlphaEvolveEvaluator:
    ...
    Args:
        evaluation_file (str): Path to a file containing a `evaluate` function
        timeout (int): Maximum evaluation time in seconds. Default: 300
        max_retries (int): Maximum number of retries for evaluation. Default: 3
        memory_limit_mb (Optional[int]): Maximum memory limit in MB.
        cpu_limit (Optional[float]): Maximum CPU limit.
        cascade_evaluation (bool): Whether to use cascade evaluation. Default: True
        cascade_thresholds (List[float]): Cascade evaluation thresholds. Default: [0.5, 0.75, 0.9]
        parallel_evaluations (int): Number of parallel evaluations. Default: 4
        distributed (bool): Whether to use distributed evaluation. Default: False
        use_llm_feedback (bool): Whether to use LLM feedback. Default: False
        llm_feedback_weight (float): Weight for LLM feedback. Default: 0.1
        llms (Optional[List[BaseLLM]]): List of LLMs to use for evaluation. Required if `use_llm_feedback` is True.
        llm_weights (Optional[List[float]]): List of weights for the LLMs. If not provided, equal weights are used.
        prompt_sampler (Optional[AlphaEvolvePromptSampler]): Prompt sampler to build evaluation prompt. Required if `use_llm_feedback` is True.
        enable_artifacts (bool): Whether to enable artifact storage. Default: True
        max_artifact_storage (int): Maximum artifact storage in bytes. Default: 100MB
    """
    ...
    def __init__(
        ...
        system_message: str = BASE_EVALUATOR_SYSTEM_TEMPLATE,
        evaluation_prompt: str = EVALUATION_TEMPLATE,
        ...
        if use_llm_feedback:
            if not llms:
                raise ValueError("LLMs must be provided if 'use_llm_feedback' is True")
            self.llm_ensemble = LLMEnsemble(llms, llm_weights)
        ...
        self.cascade_evaluation = cascade_evaluation
        ...
        self.max_artifact_storage = max_artifact_storage

        # Create a task pool for parallel evaluation
        self.task_pool = TaskPool(max_concurrency=self.parallel_evaluations)
    ...
    async def evaluate_program(
        ...
        for attempt in range(self.max_retries + 1):
            ...
            try:
                ...
                if self.use_llm_feedback and self.llm_ensemble:
                    feedback_metrics = await self._llm_evaluate(program_code)

                    # Combine metrics
                    for name, value in feedback_metrics.items():
                        eval_result.metrics[f"llm_{name}"] = value * self.llm_feedback_weight
                ...
                last_exception = e
                logger.warning(
                    f"Evaluation attempt {attempt + 1}/{self.max_retries + 1} failed for program{program_id_str}: {str(e)}"
                )
    ...
    @run_in_executor
    def _direct_evaluate(self, program_path: str) -> Union[Dict[str, float], EvaluationResult]:
        """
        ...
        try:
            ...
            # Validate result
            if not isinstance(result, (dict, EvaluationResult)):
                logger.warning(f"Evaluation result is not 'dict' or 'EvaluationResult': {result}")
                return {"error": 0.0}
    ...
    async def _cascade_evaluate(
        ...
        try:
            ...
            if len(self.cascade_thresholds) < 2 or not self._passes_threshold(
    ...
    async def _llm_evaluate(self, program_code: str) -> Dict[str, float]:
        ...
        try:
            ...
            prompt = self.evaluation_prompt.format(current_program=program_code)
