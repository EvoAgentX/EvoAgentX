...
class SequentialExecutor:
    ...
    Handles both hashable and unhashable objects by using id() as fallback.
    """
    ...
    def __init__(self):
        ...
        # Instance-specific locks (using weak references for hashable objects)
        ...
        # Fallback for unhashable objects (using id() as key)
        self._unhashable_locks: Dict[int, threading.Lock] = {}
        self._unhashable_async_locks: Dict[int, asyncio.Lock] = {}
    ...
    def get_sync_lock(self, key: str, instance: Optional[Any] = None) -> threading.Lock:
        ...
        with self._lock_manager_lock:
            if instance is not None:
                # Try to use instance as dictionary key (hashable objects)
                try:
                    # Instance-specific lock using WeakKeyDictionary
                    if instance not in self._instance_locks:
                        self._instance_locks[instance] = {}

                    if key not in self._instance_locks[instance]:
                        self._instance_locks[instance][key] = threading.RLock()

                    return self._instance_locks[instance][key]
                except TypeError:
                    # Instance is unhashable, use id() as fallback
                    instance_id = id(instance)
                    composite_key = f"{instance_id}_{key}"

                    if composite_key not in self._unhashable_locks:
                        self._unhashable_locks[composite_key] = threading.RLock(
                        )

                    return self._unhashable_locks[composite_key]
            else:
                # Global lock
                if key not in self._global_locks:
                    self._global_locks[key] = threading.RLock()
    ...
    def get_async_lock(self, key: str, instance: Optional[Any] = None) -> asyncio.Lock:
        ...
        if instance is not None:
            ...
            try:
                ...
                if composite_key not in self._unhashable_async_locks:
                    self._unhashable_async_locks[composite_key] = asyncio.Lock(
                    )

                return self._unhashable_async_locks[composite_key]
        else:
            # Global lock
    ...
    def decorator(func: F) -> F:
        ...
        def sync_wrapper(*args, **kwargs):
            ...
            if timeout is not None:
                ...
                if not acquired:
                    raise TimeoutError(
                        f"Failed to acquire lock for {func.__name__} within {timeout} seconds")
                try:
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            if timeout is not None:
                try:
                    ...
                    raise TimeoutError(
                        f"Failed to acquire async lock for {func.__name__} within {timeout} seconds")
                try:
        ...
        def smart_wrapper(*args, **kwargs):
            forced_mode = getattr(func, "_sequential_mode", "auto")
            call_mode = kwargs.pop("__sequential_mode", None)
            if call_mode in ("sync", "async", "auto"):
                forced_mode = call_mode

            if forced_mode == "sync":
                return sync_wrapper(*args, **kwargs)
            if forced_mode == "async":
                return async_wrapper(*args, **kwargs)

            import inspect as _inspect
            is_async_func = _inspect.iscoroutinefunction(func)
            try:
        ...
        def _sequential_set_mode(new_mode: str):
            if new_mode in ("sync", "async", "auto"):
                setattr(func, "_sequential_mode", new_mode)
        smart_wrapper._sequential_set_mode = _sequential_set_mode
