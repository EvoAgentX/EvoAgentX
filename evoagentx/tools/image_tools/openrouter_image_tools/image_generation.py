                ...
                for idx, im in enumerate(images):
                    try:
                        ...
                        if image_url.startswith("data:") and "," in image_url:
                            ...
                            original_filename = self._get_unique_filename(image_name or "or_gen", idx, original_ext, total_images, suffix="_origin")
                            original_save_result = self.storage_handler.save(original_filename, image_bytes)
                            if not original_save_result["success"]:
                                print(f"⚠️ Warning: Failed to save original image {idx+1}: {original_save_result.get('error', 'Unknown error')}")
                            
                            # Apply postprocessing if needed
                            if self.auto_postprocess and (needs_pp["need_resize"] or needs_pp["need_format_conversion"]):
                                print(f"🔧 Postprocessing image {idx+1}/{total_images}...")
                            ...
                            filename = self._get_unique_filename(image_name or "or_gen", idx, ext, total_images)
